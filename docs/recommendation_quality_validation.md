...
| Labels | 9,208 / 9,208 |
| Products | 1,744 |
| Batch requests | 1,288 |
| Weak/bad count | 767 |
| Weak/bad rate | 8.33% |
| High-score weak/bad count | 108 |
| High-score weak/bad rate | 1.17% |
...
- High-score rows: `7,104`
- High-score weak/bad rows: `108`
- Within-high-score weak/bad rate: `1.52%`
- Overall high-score weak/bad rate: `1.17%`
- `function_similarity < 0.40` would catch `46` weak/bad rows but also affect `1,025` non-weak rows.
- `not_same_primary` would catch `33` weak/bad rows but also affect `838` non-weak rows.
- `same_primary_set` appears in `75` high-score weak/bad rows, so primary-set equality alone is not a reliable accept signal or reject signal.
...
```text
output/recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30
```
...
python scripts\recommendation_quality_judge_batch.py validate-results `
  ...
  --output-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30 `
...
python scripts\recommendation_quality_judge_batch.py validation-report `
  --validation-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30 `
  --top-categories 10 `
  --top-patterns 7
```
...
python scripts\recommendation_quality_judge_batch.py openai-submit `
  ...
  --env-path D:\health_batch_project\.env `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30\validation_status.json `
  --require-no-active
```
...
python scripts\recommendation_quality_judge_batch.py plan-next-sample `
  --summary-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30\openai_chunk_judge_summary.json `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted202606064_p30\validation_status.json `
