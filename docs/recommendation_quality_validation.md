...
| Metric | Value |
| --- | ---: |
| Labels | 16,132 / 16,132 |
| Products | 3,158 |
| Batch requests | 2,343 |
| Weak/bad count | 1,605 |
| Weak/bad rate | 9.95% |
| High-score weak/bad count | 202 |
...
The targeted p50 sample alone pushed the aggregate weak/bad rate just over the review gate, but it still had no low-blast-radius pattern candidate. A neutral all-category p10 holdout was added next; that holdout passed on its own and brought the merged validation back inside the gate.

Latest high-score weak diagnostics also support keeping the current algorithm:

- High-score rows: `11,834`
- High-score weak/bad rows: `202`
- Within-high-score weak/bad rate: `1.71%`
- Overall high-score weak/bad rate: `1.25%`
- `function_similarity < 0.40` would catch `82` weak/bad rows but also affect `1,631` non-weak rows.
- `not_same_primary` would catch `56` weak/bad rows but also affect `1,355` non-weak rows.
- `same_primary_set` appears in `146` high-score weak/bad rows, so primary-set equality alone is not a reliable accept signal or reject signal.
...
```text
output/recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10
```
...
python scripts\recommendation_quality_judge_batch.py validate-results `
  ...
  --output-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10 `
...
python scripts\recommendation_quality_judge_batch.py validation-report `
  --validation-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10 `
  --top-categories 10 `
  --top-patterns 7
```
...
python scripts\recommendation_quality_judge_batch.py openai-submit `
  ...
  --env-path D:\health_batch_project\.env `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10\validation_status.json `
  --require-no-active
```
...
python scripts\recommendation_quality_judge_batch.py plan-next-sample `
  --summary-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10\openai_chunk_judge_summary.json `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout202606067_p10\validation_status.json `
