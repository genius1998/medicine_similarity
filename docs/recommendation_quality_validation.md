...
| Metric | Value |
| --- | ---: |
| Labels | 42,941 / 42,941 |
| Products | 8,387 |
| Batch requests | 6,015 |
| Weak/bad count | 3,667 |
| Weak/bad rate | 8.54% |
| High-score weak/bad count | 537 |
...
A neutral all-category p45 holdout was added after that and split into three OpenAI Batch jobs to stay under the `gpt-5-nano` 2M enqueued-token limit. The three parts had standalone weak/bad rates of `4.16%`, `8.50%`, and `10.09%`; part 3 slightly exceeded the standalone weak-rate gate, but its high-score weak/bad rate was `1.32%` and it also found no actionable low-blast-radius pattern. After retrying malformed judge outputs from earlier holdouts, the merged validation has complete label coverage and remains inside the gate.

Latest high-score weak diagnostics also support keeping the current algorithm:

- High-score rows: `33,442`
- High-score weak/bad rows: `536`
- Within-high-score weak/bad rate: `1.60%`
- Overall high-score weak/bad rate: `1.25%`
- `function_similarity < 0.40` would catch `217` weak/bad rows but also affect `3,899` non-weak rows.
- `not_same_primary` would catch `127` weak/bad rows but also affect `3,420` non-weak rows.
- `same_primary_set` appears in `409` high-score weak/bad rows, so primary-set equality alone is not a reliable accept signal or reject signal.
...
```text
output/recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45
```
...
python scripts\recommendation_quality_judge_batch.py validate-results `
  ...
  --output-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45 `
...
python scripts\recommendation_quality_judge_batch.py validation-report `
  --validation-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45 `
  --top-categories 10 `
  --top-patterns 7
```
...
python scripts\recommendation_quality_judge_batch.py openai-submit `
  ...
  --env-path D:\health_batch_project\.env `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45\validation_status.json `
  --require-no-active
```
...
python scripts\recommendation_quality_judge_batch.py plan-next-sample `
  --summary-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45\openai_chunk_judge_summary.json `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060616_p45\validation_status.json `
