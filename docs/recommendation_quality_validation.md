...
| Metric | Value |
| --- | ---: |
| Labels | 73,147 / 73,147 |
| Products | 14,715 |
| Batch requests | 10,325 |
| Weak/bad count | 6,299 |
| Weak/bad rate | 8.61% |
| High-score weak/bad count | 987 |
| High-score weak/bad rate | 1.35% |
...
A targeted p25 sample with seed `2026060710` was then run against the current highest weak-rate categories: 여성 건강, 영양보충, 수면/긴장완화, 혈중지질, 피로개선, 혈당, 혈압, and 체지방. It covered `200` products and produced `133` OpenAI Batch requests. Its standalone weak/bad rate was elevated at `15.48%`, as expected for those categories, but standalone high-score weak/bad rate stayed within the gate at `1.26%` and no actionable low-blast-radius pattern was found. The merged validation still passes the quality gate.

A targeted high-score-weak p25 sample with seed `2026060711` was then run against categories where high-score weak rows were concentrated: 혈당, 피부 건강, 남성 건강, 피로개선, 구강 건강, 혈중지질, 간 건강, and 뼈 건강. It covered `200` products and produced `165` OpenAI Batch requests. Its standalone weak/bad rate was `6.65%`; standalone high-score weak/bad rate was `1.99%`, right under the standalone gate, but no actionable low-blast-radius pattern was found. The merged validation still passes the quality gate, and the new rows again show that broad caps would remove many reasonable or acceptable-adjacent recommendations.

A stricter high-score p25 sample with seed `2026060712` was then run against categories with many `>=0.85` high-score weak rows: 뼈 건강, 피부 건강, 간 건강, 남성 건강, 눈 건강, 구강 건강, 혈행, and 기타. It covered `200` products and produced `162` OpenAI Batch requests. One OpenAI response omitted `10` labels, so that base product was retried separately. After the retry, the sample had complete label coverage. Its standalone weak/bad rate was `5.68%`, standalone high-score weak/bad rate was `1.92%`, and no actionable low-blast-radius pattern was found.

A neutral all-category p20 holdout with seed `2026060713` was then run after the high-score stress samples to check whole-distribution stability again. It covered `447` products, produced `333` OpenAI Batch requests, and had complete label coverage. Its standalone weak/bad rate was `7.46%`, standalone high-score weak/bad rate was `1.53%`, and no actionable low-blast-radius pattern was found.

A low-coverage mixed p30 sample with seed `2026060714` was then run against 영양보충, 기타, 운동/근력, 항산화, 기억력, 여성 건강, 혈압, and 혈중지질. It covered `240` products and produced `117` OpenAI Batch requests. Its standalone weak/bad rate was `10.55%`, slightly over the standalone gate, but standalone high-score weak/bad rate was only `0.72%` and no actionable low-blast-radius pattern was found. The merged validation still passes, so this sample does not justify an algorithm cap.

A weak-confirmation p35 sample with seed `2026060715` was then run against 여성 건강, 혈압, 혈중지질, 기억력, 항산화, 수면/긴장완화, 피로개선, and 혈당. It covered `280` products and produced `215` OpenAI Batch requests. Its standalone weak/bad rate was `12.68%`, but standalone high-score weak/bad rate stayed within the gate at `1.15%`; no actionable low-blast-radius pattern was found. The merged validation still passes, so this elevated targeted weak rate still does not justify a broad cap.

A targeted follow-up p40 sample with seed `2026060716` was then run against the highest weak-rate categories: 여성 건강, 영양보충, 수면/긴장완화, 혈중지질, 피로개선, 혈압, 혈당, and 체지방. It covered `320` products and produced `212` OpenAI Batch requests. One `max_output_tokens` response was retried for complete label coverage. The standalone weak/bad rate was `11.02%`, but standalone high-score weak/bad rate stayed within the gate at `1.14%`; no actionable low-blast-radius pattern was found. The merged validation still passes, so this targeted follow-up again supports keeping the current algorithm.

Latest high-score weak diagnostics also support keeping the current algorithm:

- High-score rows: `56,900`
- High-score weak/bad rows: `986`
- Within-high-score weak/bad rate: `1.73%`
- Overall high-score weak/bad rate: `1.35%`
- `function_similarity < 0.40` would catch `396` weak/bad rows but also affect `6,518` non-weak rows.
- `not_same_primary` would catch `244` weak/bad rows but also affect `5,962` non-weak rows.
...
```text
output/recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40
```
...
output/recommendation_quality_judge_v2_9_openai_targeted_next_seed2026060710_p25
output/recommendation_quality_judge_v2_9_openai_targeted_highscoreweak_seed2026060711_p25
output/recommendation_quality_judge_v2_9_openai_targeted_strict_highscore_seed2026060712_p25
output/recommendation_quality_judge_v2_9_openai_holdout_seed2026060713_p20
output/recommendation_quality_judge_v2_9_openai_targeted_lowcoverage_mixed_seed2026060714_p30
...
output/recommendation_quality_judge_v2_9_openai_targeted_strict_highscore_seed2026060712_p25_retry_*
output/recommendation_quality_judge_v2_9_openai_targeted_followup_seed2026060716_p40_retry_*
```
...
python scripts\recommendation_quality_judge_batch.py validate-results `
  ...
  --parts-glob "output\recommendation_quality_judge_v2_9_openai_targeted_next_seed2026060710_p25" `
  --parts-glob "output\recommendation_quality_judge_v2_9_openai_targeted_highscoreweak_seed2026060711_p25" `
  --parts-glob "output\recommendation_quality_judge_v2_9_openai_targeted_strict_highscore_seed2026060712_p25" `
  --parts-glob "output\recommendation_quality_judge_v2_9_openai_holdout_seed2026060713_p20" `
  --parts-glob "output\recommendation_quality_judge_v2_9_openai_targeted_lowcoverage_mixed_seed2026060714_p30" `
  ...
  --output-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40 `
...
python scripts\recommendation_quality_judge_batch.py validation-report `
  --validation-dir output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40 `
  --top-categories 10 `
  --top-patterns 7
```
...
python scripts\recommendation_quality_judge_batch.py openai-submit `
  ...
  --env-path D:\health_batch_project\.env `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40\validation_status.json `
  --require-no-active
```
...
python scripts\recommendation_quality_judge_batch.py plan-next-sample `
  --summary-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40\openai_chunk_judge_summary.json `
  --validation-status-json output\recommendation_quality_judge_v2_9_openai_validation_current_plus_targeted_holdout2026060716_followup_p40\validation_status.json `
