...
def default_main_category_overrides_path() -> str:
    path = ROOT_DIR / "output" / "final_all_result_category_overrides.csv"
    return str(path) if path.exists() else ""


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build product function profiles from product vectors.")
    parser.add_argument(
        "--main-category-overrides",
        default=default_main_category_overrides_path(),
        help=(
            "Optional CSV generated by scripts/classify_product_main_categories_llm.py. "
            "Rows that pass the confidence/decision gates override product_main_category and "
            "product_sub_categories before role assignment/output."
        ),
    )
    parser.add_argument("--override-min-confidence", type=float, default=0.9)
    parser.add_argument(
        "--override-decisions",
        default="auto_override_candidate",
        help="Comma-separated decision values allowed from the override CSV.",
    )
...
def invalidate_similarity_cache_for_report_nos(conn: sqlite3.Connection, report_nos: set[str]) -> int:
    if not report_nos:
        return 0
    existing = conn.execute(
        "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
        ("product_similarity_explanation_cache",),
    ).fetchone()
    if not existing:
        return 0

    product_ids = [f"품목제조번호::{report_no}" for report_no in sorted(report_nos) if str(report_no).strip()]
    if not product_ids:
        return 0
    if len(product_ids) >= 10000:
        cursor = conn.execute("DELETE FROM product_similarity_explanation_cache")
        conn.commit()
    ...
    for start in range(0, len(product_ids), chunk_size):
        ...
        cursor = conn.execute(
            f"""
            DELETE FROM product_similarity_explanation_cache
            WHERE base_product_id IN ({placeholders})
               OR target_product_id IN ({placeholders})
            """,
            chunk + chunk,
        )
...
def load_main_category_overrides(
    path: Path | str,
    *,
    ...
    if not override_path.exists():
        raise FileNotFoundError(f"main category override CSV not found: {override_path}")

    df = pd.read_csv(override_path, encoding="utf-8-sig", low_memory=False).fillna("")
    required_columns = {"report_no", "confidence"}
    category_column = "predicted_main_category" if "predicted_main_category" in df.columns else "forced_main_category"
    sub_category_column = ""
    for candidate in ["predicted_sub_categories", "forced_sub_categories_json", "forced_sub_categories"]:
        if candidate in df.columns:
            sub_category_column = candidate
            break
    sub_function_category_column = ""
    for candidate in [
        "predicted_sub_function_categories",
        "forced_sub_function_categories_json",
        "forced_sub_function_categories",
    ]:
        if candidate in df.columns:
            sub_function_category_column = candidate
            break
    if category_column not in df.columns:
        raise ValueError("override CSV requires predicted_main_category or forced_main_category")
    missing = [column for column in required_columns if column not in df.columns]
    if missing:
        raise ValueError(f"override CSV missing columns: {missing}")

    df["report_no"] = df["report_no"].astype(str).str.strip()
    df[category_column] = df[category_column].astype(str).str.strip()
    df["confidence"] = pd.to_numeric(df["confidence"], errors="coerce").fillna(0.0)
    if "decision" not in df.columns:
        df["decision"] = "auto_override_candidate"
    df["decision"] = df["decision"].astype(str).str.strip()

    overrides: dict[str, dict] = {}
    for row in df.to_dict(orient="records"):
        report_no = str(row.get("report_no", "")).strip()
        forced_category = str(row.get(category_column, "")).strip()
        forced_sub_categories = parse_json_list(row.get(sub_category_column, "")) if sub_category_column else []
        forced_sub_function_categories = (
            parse_json_list(row.get(sub_function_category_column, ""))
            if sub_function_category_column
            else []
        )
        decision = str(row.get("decision", "")).strip()
        confidence = float(row.get("confidence", 0.0) or 0.0)
        if not report_no or (not forced_category and not forced_sub_categories and not forced_sub_function_categories):
            continue
        ...
        if allowed_decisions and decision not in allowed_decisions:
            continue
        overrides[report_no] = {
            "forced_main_category": forced_category,
            "forced_sub_categories": forced_sub_categories,
            "forced_sub_function_categories": forced_sub_function_categories,
            "confidence": round(confidence, 4),
            "decision": decision,
            "source": str(row.get("source", "") or "llm_main_category_classification"),
...
def compute_profile_for_product(product_id: str, group: pd.DataFrame, main_category_overrides: dict | None = None) -> dict:
    ...
    computed_product_main_category = sorted_categories[0][0] if sorted_categories else "기타"
    product_main_category = computed_product_main_category
    if product_name_is_memory and has_phosphatidylserine:
        ...
        product_main_category = "남성 건강"
    computed_product_main_category = product_main_category
    override_info = (main_category_overrides or {}).get(report_no, {})
    main_override_applied = bool(override_info.get("forced_main_category"))
    if main_override_applied:
        product_main_category = str(override_info.get("forced_main_category", "")).strip()
    forced_sub_categories = list(override_info.get("forced_sub_categories", []) or [])
    sub_override_applied = bool(forced_sub_categories)
    forced_sub_function_categories = [
        category
        for category in list(override_info.get("forced_sub_function_categories", []) or [])
        if str(category).strip() and str(category).strip() != product_main_category
    ]
    sub_function_override_applied = bool(forced_sub_function_categories)
    ...
    if main_override_applied:
        notes.append(
            "메인 카테고리 override 적용: "
            f"{computed_product_main_category} -> {product_main_category}"
            f" (source={override_info.get('source', '')}, "
            f"confidence={override_info.get('confidence', '')}, "
            f"reason={override_info.get('reason', '')})"
        )
    computed_sub_categories = list(sub_categories)
    if sub_override_applied:
        notes.append(
            "서브카테고리 override 적용: "
            f"{computed_sub_categories} -> {forced_sub_categories}"
            f" (source={override_info.get('source', '')}, "
            f"confidence={override_info.get('confidence', '')}, "
            f"reason={override_info.get('reason', '')})"
        )
        sub_categories = forced_sub_categories
    if sub_function_override_applied:
        notes.append(
            "보조 기능 카테고리 override 적용: "
            f"{forced_sub_function_categories}"
            f" (source={override_info.get('source', '')}, "
            f"confidence={override_info.get('confidence', '')}, "
            f"reason={override_info.get('reason', '')})"
        )
    ...
    computed_category_scores_json = {key: round(float(value), 6) for key, value in sorted_categories}
    final_category_scores = dict(computed_category_scores_json)
    if main_override_applied:
        current_top_score = max(final_category_scores.values(), default=0.0)
        final_category_scores[product_main_category] = round(
            max(float(final_category_scores.get(product_main_category, 0.0)), current_top_score + 0.001, 1.0),
            6,
        )
    if sub_function_override_applied:
        main_score = float(final_category_scores.get(product_main_category, max(final_category_scores.values(), default=1.0)))
        for idx, category in enumerate(forced_sub_function_categories):
            weight = max(0.35, 0.72 - (idx * 0.07))
            final_category_scores[category] = round(
                max(float(final_category_scores.get(category, 0.0)), main_score * weight),
                6,
            )
    category_scores_json = dict(sorted(final_category_scores.items(), key=lambda item: (-float(item[1]), item[0])))
    ...
    return {
        ...
        "computed_product_main_category": computed_product_main_category,
        "main_category_override_applied": int(main_override_applied),
        "main_category_override_source": str(override_info.get("source", "")) if main_override_applied else "",
        "main_category_override_confidence": float(override_info.get("confidence", 0.0) or 0.0) if main_override_applied else 0.0,
        "main_category_override_reason": str(override_info.get("reason", "")) if main_override_applied else "",
        "computed_product_sub_categories_json": to_json(computed_sub_categories),
        "sub_category_override_applied": int(sub_override_applied),
        "llm_sub_function_categories_json": to_json(forced_sub_function_categories),
        "sub_function_category_override_applied": int(sub_function_override_applied),
        ...
        "computed_category_scores_json": to_json(computed_category_scores_json),
...
def main() -> None:
    ...
    allowed_override_decisions = {
        item.strip()
        for item in str(args.override_decisions or "").split(",")
        if item.strip()
    }
    main_category_overrides = load_main_category_overrides(
        args.main_category_overrides,
        min_confidence=float(args.override_min_confidence),
        allowed_decisions=allowed_override_decisions,
    )
    if main_category_overrides:
        print(f"[INFO] main_category_overrides={len(main_category_overrides)}")
    ...
    for idx, (product_id, group) in enumerate(grouped, start=1):
        try:
            row = compute_profile_for_product(product_id, group.copy(), main_category_overrides)
            rows.append(row)
            ...
            failed_rows.append(
                {
                    ...
                    "computed_product_main_category": "",
                    "main_category_override_applied": 0,
                    "main_category_override_source": "",
                    "main_category_override_confidence": 0.0,
                    "main_category_override_reason": "",
                    "computed_product_sub_categories_json": "[]",
                    "sub_category_override_applied": 0,
                    "llm_sub_function_categories_json": "[]",
                    "sub_function_category_override_applied": 0,
                    "product_sub_categories_json": "[]",
                    "primary_ingredients_json": "[]",
                    "secondary_ingredients_json": "[]",
                    "support_ingredients_json": "[]",
                    "category_scores_json": "{}",
                    "computed_category_scores_json": "{}",
    ...
    with sqlite3.connect(runtime["sqlite_path"]) as conn:
        ...
        invalidated_cache_rows = invalidate_similarity_cache_for_report_nos(conn, set(main_category_overrides))
        if invalidated_cache_rows:
            print(f"[INFO] invalidated_similarity_cache_rows={invalidated_cache_rows}")
    ...
    summary = {
        ...
        "main_category_override_csv": str(args.main_category_overrides or ""),
        "main_category_override_count": int(len(main_category_overrides)),
        "main_category_override_applied_count": int(result_df["main_category_override_applied"].sum()) if "main_category_override_applied" in result_df else 0,
        "sub_category_override_applied_count": int(result_df["sub_category_override_applied"].sum()) if "sub_category_override_applied" in result_df else 0,
        "sub_function_category_override_applied_count": int(result_df["sub_function_category_override_applied"].sum()) if "sub_function_category_override_applied" in result_df else 0,
        "invalidated_similarity_cache_rows": int(invalidated_cache_rows),
